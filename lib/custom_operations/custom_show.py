...
def vis_text_beautiful(im, str_list):
    ...
    gpu_name, mem_used, mem_total, detect_time, nms_time, total_time, frame_rate = str_list

    show_string_1 = 'device: %s         memory: %.3f G / %.3f G' % (gpu_name, mem_used, mem_total)
    show_string_2 = 'detect: %.3fs    nms: %.3fs    total: %.3fs    FPS: %.3f' % (detect_time, nms_time, total_time, frame_rate)
